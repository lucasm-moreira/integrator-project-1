...
mediaValorPrincipioAtivo = principioAtivoTibble %>%
  group_by(ID, NOME) %>%
  summarise("VALOR MÍNIMO" = round(min(VALOR), 2),
            "VALOR MÉDIO" = round(mean(VALOR), 2),
            "VALOR MÁXIMO" = round(max(VALOR), 2),
            "ANO" = 2020) %>%
  arrange(desc(`VALOR MÉDIO`))

minValorPrincipioAtivo = mediaValorPrincipioAtivo %>%
  arrange(`VALOR MÍNIMO`)

maxValorPrincipioAtivo = mediaValorPrincipioAtivo %>%
  arrange(desc(`VALOR MÁXIMO`))
...
minValorPrincipioAtivoPlot <- ggplot(data = minValorPrincipioAtivo) + 
  ...
  labs(x = "Ano", y = "Valor Mínimo (R$)") +
  theme(axis.title = element_text(size=10), 
        plot.title = element_text(size=12, face="bold")) +
  ggtitle("Valor Mínimo cobrado por Princípio Ativo - 2020 (Top 10)")
...
maxValorPrincipioAtivoPlot <- ggplot(data = maxValorPrincipioAtivo) + 
  ...
  labs(x = "Ano", y = "Valor Máximo (R$)") +
  theme(axis.title = element_text(size=10), 
        plot.title = element_text(size=12, face="bold")) +
  ggtitle("Valor Máximo cobrado por Princípio Ativo - 2020 (Top 10)")
